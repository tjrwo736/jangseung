...
from src.evidence import phase11a_completion_baseline
from src.evidence.phase11a_completion_baseline import (
...
class Phase11aCompletionBaselineTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_valid_11a0_11a1_11a2_11a3_chain_builds_completion_baseline(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)
        verify = self._verify(record, source0, source1, source2, source3)

        self.assertTrue(verify["accepted"])
        self.assertEqual(verify["status"], PHASE11A_COMPLETION_BASELINE_ACCEPTED)
        self.assertEqual(record["record_kind"], PHASE11A_COMPLETION_BASELINE_KIND)
        self.assertEqual(record["record_version"], PHASE11A_COMPLETION_BASELINE_VERSION)
        self.assertEqual(record["phase11a_step"], PHASE11A_STEP)
        self.assertEqual(record["completion_status"], PHASE11A_COMPLETION_BASELINE_ACCEPTED)
        self.assertEqual(
            record["completion_basis"],
            COMPLETION_BASIS_PHASE11A_INTERNAL_RECOMPUTE_CLOSURE,
        )
        self.assertEqual(record["completion_chain_binding_result"], COMPLETION_CHAIN_BINDING_ACCEPTED)
        ...
        self.assertEqual(record["phase11a3_verify_status"], EVIDENCE_BINDING_ACCEPTED)
        self.assertEqual(record["phase11a3_binding_status"], EVIDENCE_BINDING_ACCEPTED)
        self.assertTrue(record["source_verify_recomputed"])
        self.assertFalse(record["supplied_verify_trusted"])
        self.assertTrue(record["completion_chain_binding_valid"])
        self.assertTrue(record["phase11a3_deterministic_binding_verified"])
        self.assertTrue(record["phase11a2_internal_route_verify_preserved"])
        ...
        self.assertTrue(record["candidate_success_rejected_as_rollback_evidence"])
        ...
        self.assertFalse(record["runtime_write_authority_granted"])
    ...
    def test_source_11a0_tamper_rejects(self):
        ...
        verify = self._verify(record, tampered0, source1, source2, source3)
    ...
    def test_source_11a1_tamper_rejects(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)
        tampered1 = self._source1_with(source1, save_run_route_target="other_route")

        verify = self._verify(record, source0, tampered1, source2, source3)
    ...
    def test_source_11a2_tamper_rejects(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)
        tampered2 = self._source2_with(source2, known_gap_transition_status="OTHER_TRANSITION")

        verify = self._verify(record, source0, source1, tampered2, source3)
    ...
    def test_source_11a3_tamper_rejects(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)
        tampered3 = self._source3_with(source3, binding_status="BROKEN")

        verify = self._verify(record, source0, source1, source2, tampered3)

        self.assert_rejected_with(verify, "source 11-A-3 evidence digest mismatch")
        self.assert_rejected_with(verify, "source 11-A-3 evidence verify rejected")

    def test_source_digest_mismatch_rejects(self):
        source0, source1, source2, source3 = self._chain()
        record = deepcopy(self._baseline(source0, source1, source2, source3))
        record["source_phase11a3_digest"] = "bad-digest"
    ...
    def test_supplied_verify_object_is_not_trusted(self):
        ...
        supplied3 = verify_phase11a_write_route_evidence_binding(
            source3,
        ...
        record = self._baseline(
            ...
            supplied_phase11a3_verify=supplied3,
        )
        verify = self._verify(
            record,
            source0,
    ...
    def test_fake_source_verify_accepted_object_cannot_pass_invalid_source_evidence(self):
        ...
        self.assertEqual(record["completion_status"], SOURCE_EVIDENCE_VERIFY_REJECTED)
        self.assert_rejected_with(verify, "source 11-A-1 evidence verify rejected")
    ...
    def test_11a2_supplied_verify_bypass_remains_rejected(self):
        source0, source1, _source2, _source3 = self._chain()
        tampered1 = deepcopy(source1)
        tampered1["deterministic_evidence_digest"] = "bad-digest"
        fake_accepted = self._fake_verify(status=SAVE_RUN_VERIFY_REPLAY_ACCEPTED)
    ...
    def test_11a3_binding_broken_rejects(self):
        source0, source1, source2, source3 = self._chain()
        broken3 = self._source3_with(
            source3,
            cross_digest_binding_valid=False,
            completion_chain_binding_valid=True,
        )

        record = self._baseline(source0, source1, source2, broken3)
        verify = self._verify(record, source0, source1, source2, broken3)

        self.assert_rejected_with(verify, "source 11-A-3 evidence verify rejected")
        self.assert_rejected_with(verify, "completion chain binding mismatch")
        self.assert_rejected_with(verify, "11-A-3 deterministic binding broken")
    ...
    def test_deterministic_attribution_mismatch_rejects(self):
        ...
        source3 = self._binding(source0, tampered1, source2)
        verify = self._verify(self._baseline(source0, tampered1, source2, source3), source0, tampered1, source2, source3)
    ...
    def test_candidate_success_accepted_as_11a0_rollback_evidence_rejects(self):
        _source0, source1, _source2, _source3 = self._chain()
    ...
    def test_known_gap_transition_mismatch_rejects(self):
        source0, source1, source2, _source3 = self._chain()
        tampered2 = self._source2_with(
            source2,
            known_gap_transition_status="KNOWN_GAP_TRANSITION_REJECTED",
        )
        source3 = self._binding(source0, source1, tampered2)
        verify = self._verify(self._baseline(source0, source1, tampered2, source3), source0, source1, tampered2, source3)
    ...
    def test_runtime_write_authority_grant_claim_rejects(self):
        ...
        tampered3 = self._source3_with(source3, **{grant_key: True})
        verify = self._verify(self._baseline(source0, source1, source2, tampered3), source0, source1, source2, tampered3)
    ...
    def test_live_executor_authority_grant_claim_rejects(self):
        ...
        tampered3 = self._source3_with(source3, live_executor_authority="PROMOTED", **{grant_key: True})
        verify = self._verify(self._baseline(source0, source1, source2, tampered3), source0, source1, source2, tampered3)
    ...
    def test_phase11b_claim_rejects(self):
        source0, source1, source2, source3 = self._chain()
        tampered3 = self._source3_with(source3, phase11b_status="STARTED", phase11b_started=True)
        verify = self._verify(self._baseline(source0, source1, source2, tampered3), source0, source1, source2, tampered3)
    ...
    def test_actual_enforcement_claim_rejects(self):
        source0, source1, source2, source3 = self._chain()
        record = deepcopy(self._baseline(source0, source1, source2, source3))
        record["actual_enforcement_activated"] = True
        record["deterministic_evidence_digest"] = phase11a_completion_baseline_digest(record)
    ...
    def test_11a0_11a1_11a2_11a3_preservation_passes(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)

        self.assertEqual(
            verify_phase11a_rollback_kill_path_evidence(source0)["status"],
            ROLLBACK_VERIFY_REPLAY_ACCEPTED,
        )
        self.assertEqual(
            verify_phase11a_save_run_write_routing_evidence(source1)["status"],
            SAVE_RUN_VERIFY_REPLAY_ACCEPTED,
        )
        self.assertEqual(
            verify_phase11a_known_gap_blocked_transition_evidence(
                source2,
            ...
            KNOWN_GAP_VERIFY_REPLAY_ACCEPTED,
        )
        self.assertEqual(
            verify_phase11a_write_route_evidence_binding(
                source3,
        ...
        self.assertEqual(
            source1["write_attribution_basis"],
            WRITE_ATTRIBUTION_BASIS_DETERMINISTIC_ADAPTER_CONTEXT,
        )
        self.assertEqual(source2["known_gap_transition_status"], KNOWN_GAP_BLOCKED_BY_PRELIVE_ROUTE)
        self.assertTrue(record["completion_chain_binding_valid"])

    def test_completion_baseline_digest_is_bound(self):
        source0, source1, source2, source3 = self._chain()
        record = self._baseline(source0, source1, source2, source3)

        self.assertEqual(
            record["deterministic_evidence_digest"],
            phase11a_completion_baseline_digest(record),
        )

        tampered = deepcopy(record)
        tampered["completion_basis"] = "other_basis"
        verify = self._verify(tampered, source0, source1, source2, source3)
    ...
    def test_module_does_not_add_runtime_side_effect_helpers(self):
        source = inspect.getsource(phase11a_completion_baseline)
        forbidden_calls = (
            "." + "open(",
            "op" + "en(",
            ".write" + "_text(",
    ...
    def _chain(self):
        ...
        source3 = self._binding(source0, source1, source2)
        return source0, source1, source2, source3
    ...
    def _route(self):
        request = build_save_run_write_request(
            ...
            payload_metadata={"source": "phase11a4_test_double"},
            request_id="phase11a-completion-baseline-test-request",
        )
    ...
    def _baseline(self, source0, source1, source2, source3, **kwargs):
        return build_phase11a_completion_baseline(
            phase11a0_evidence=source0,
            phase11a1_evidence=source1,
            phase11a2_evidence=source2,
            phase11a3_binding_evidence=source3,
            **kwargs,
        )

    def _verify(self, record, source0, source1, source2, source3, **kwargs):
        return verify_phase11a_completion_baseline(
            record,
            phase11a0_evidence=source0,
            phase11a1_evidence=source1,
            phase11a2_evidence=source2,
    ...
    def _source2_with(self, source2, **changes):
        tampered = deepcopy(source2)
        tampered.update(changes)
        tampered["deterministic_evidence_digest"] = phase11a_known_gap_transition_evidence_digest(tampered)
        return tampered

    def _source3_with(self, source3, **changes):
        tampered = deepcopy(source3)
        tampered.update(changes)
        tampered["deterministic_evidence_digest"] = phase11a_write_route_evidence_binding_digest(tampered)
        return tampered
